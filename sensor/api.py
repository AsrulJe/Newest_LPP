...
@api_view(['GET'])
@permission_classes((AllowAny,))
def api_get_timer_per_batch(request, dryer_id):
    batch_collection = 'batch_dryer_' + str(dryer_id)
...
@api_view(['PUT'])
@permission_classes((AllowAny,))
def api_put_sampling(request, dryer_id):

    batch_collection = 'batch_dryer' + str(dryer_id)
    ...
    # func.updateOneItem({'batch_id': batch_id}, batch_collection)
    return Response({"message": "Success"})
